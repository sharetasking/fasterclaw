...
const Main = ({ instance: initialInstance }: MainProps) => {
  ...
  useEffect(() => {
    return () => {
      if (attachedFile?.previewUrl != null && attachedFile.previewUrl !== "") {
  ...
  const handleSend = async () => {
    ...
    if ((!hasMessage && !hasFile) || isLoading || isUploading || instance?.status !== "RUNNING") {
      return;
    }

    // Convert blob URL to a persistent data URL before clearing attachedFile,
    // otherwise the cleanup effect revokes the object URL and breaks the image.
    let persistentPreview: string | undefined;
    if (attachedFile?.isImage === true && attachedFile.previewUrl != null) {
      try {
        const response = await fetch(attachedFile.previewUrl);
        const blob = await response.blob();
        persistentPreview = await new Promise<string>((resolve, reject) => {
        ...
        persistentPreview = attachedFile.previewUrl;
      }
    }
    ...
    const userMsg: ChatMessage = {
      ...
      imagePreview: persistentPreview,
    };
    ...
    try {
      const result = await sendChatMessage(
        ...
        hasMessage
          ? userMsg.content
          : `Please analyze the attached file: ${attachedFile?.name ?? "file"}`,
        filePath
      );
      if (result.success) {
        setMessages((prev) => [
  ...
  const isReady = instance?.status === "RUNNING";
      ...
      >
        {messages.length === 0 ? (
          <>
            {isProvisioning && (
              <div className="mb-10 text-center">
                ...
                <div className="body1 text-n-4 2xl:body1S">
                  This usually takes about 30 seconds. You can start chatting once it&apos;s ready.
                </div>
            ...
            {isFailed && (
              <div className="mb-10 text-center">
                <div className="h3 leading-[4rem] 2xl:mb-2 2xl:h4">Setup failed</div>
                <div className="body1 text-n-4 2xl:body1S">
                  Please visit the Dashboard to retry instance creation.
                </div>
              </div>
            )}
            {(isReady || !instance) && (
              <>
                <div className="mb-10 text-center">
                  <div className="h3 leading-[4rem] 2xl:mb-2 2xl:h4">
                    {instance ? "Start a conversation" : "Unlock the power of AI"}
                  </div>
                  <div className="body1 text-n-4 2xl:body1S">
                    {instance
                ...
                {!instance && <Menu className="max-w-[30.75rem] mx-auto" items={navigation} />}
              </>
            )}
          </>
        ) : (
            ...
            {messages.map((msg, i) =>
              msg.role === "user" ? (
                <Question
                  ...
                  content={msg.content}
                  document={
                    msg.fileName != null && msg.fileName !== "" && msg.isImage !== true
                      ? msg.fileName
                      : undefined
                  }
                ...
                <Answer key={i} time={formatTime(msg.timestamp)}>
                  {msg.content}
                </Answer>
              )
            )}
